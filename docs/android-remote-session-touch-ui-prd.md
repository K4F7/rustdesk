1. # Product Requirements Document：Android 远控会话触控交互与虚拟按键增强 (v1.1)
   ...
   **验收标准：**
   - 单指长按触发右键单击。
   - 右键单击的判定口径：长按进入“右键按住”状态；松手时若未进入拖动，则发送右键抬起并形成右键单击。
   ...
   **验收标准：**
   - 单指拖动触发左键按住移动，松手后抬起。
   - 必须通过 U2 验证：Input Event Log 显示 `left_drag` 序列（Down -> Move -> Up，推荐用 `phase` 字段断言）。

   ### Story 4：长按后快速拖动=鼠标右键按住拖动
   **验收标准：**
   - 长按后保持按住并拖动，触发右键按住移动。
   - 与 Story 2 互斥：进入拖动状态后，松手时不触发右键单击。
   - 必须通过 U2 验证：Input Event Log 显示 `right_drag` 序列（Down -> Move -> Up，推荐用 `phase` 字段断言），且无 `right_click`。
 ...
 - **按键“卡住”风险**：右键按住/拖动过程中若发生中断（系统取消、意外多指介入等），需确保最终会发送 RightUp 以避免远端按键卡住。
